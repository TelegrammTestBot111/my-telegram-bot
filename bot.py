                ...
                if days_list and is_scheduled_day(days_list):
                    if now.hour == sched["hour"] and now.minute >= sched["minute"]:
                        if data.get("last_reminder_date") != today_str:
                            try:
                                await bot.send_message(user_id, f"⏰ Напоминание по расписанию!\nВремя: {sched['hour']}:{sched['minute']}")
                                data["last_reminder_date"] = today_str
                            except Exception as e:
                                logging.error(f"Ошибка (расписание): {e}")
            ...
            for i in range(len(reminders)):
                ...
                if now >= reminder["execute_at"]:
                    try:
                        ...
                        logging.error(f"Ошибка (разовое): {e}")
...
async def handle_any_message(message: types.Message):
    ...
    if one_time_match:
        ...
        delay = timedelta(minutes=amount) if "мин" in unit else timedelta(hours=amount)
        execute_at = datetime.now() + delay
        users[user_id]["one_time_reminders"].append({"text": text, "execute_at": execute_at})
    ...
    for part in parts:
        ...
        if day_found and time_match:
            hour = int(time_match.group(1))
            minute = int(time_match.group(2))
            new_schedules.append({"days": [day_found], "hour": hour, "minute": minute})
            found_any = True
